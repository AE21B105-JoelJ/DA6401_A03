# Train script for Vanilla Seq2Seq Model #
...
parser = argparse.ArgumentParser(description="Trainin a Vanilla Seq2Seq model !!!")
...
parser.add_argument("-dropout", "--dropout", type=float, default=0.4, help="Dropout to applu at the convolutional layers before activation")
...
parser.add_argument("-num_layers", "--num_layers", type=int, default=5, help="Determine the number of layers")
parser.add_argument("-embed_dim", "--embed_dim", type=int, default=64, help="Determine the embedding dimension")
parser.add_argument("-hidden_size", "--hidden_size", type=int, default=256, help="Determine the hidden dimension")
parser.add_argument("-enc_cell", "--enc_cell", type=str, default="GRU", help="Determine the encoder cell type")
parser.add_argument("-dec_cell", "--dec_cell", type=str, default="LSTM", help="Determine the decoder cell type")
...
def main_wrapper(config = None):
    ...
    if args.pre_trained:
        model.load_state_dict(torch.load(os.path.join(os.path.abspath(""), "pretrained/Vanilla_Best_model.pth"), weights_only=True))
    else:
